...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.rombonganBelajar.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.rombongan-belajars.update", [$rombonganBelajar->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="nama_rombel">{{ trans('cruds.rombonganBelajar.fields.nama_rombel') }}</label>
                <input class="form-control {{ $errors->has('nama_rombel') ? 'is-invalid' : '' }}" type="text" name="nama_rombel" id="nama_rombel" value="{{ old('nama_rombel', $rombonganBelajar->nama_rombel) }}" required>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\RombonganBelajar::JURUSAN_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('jurusan') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="jurusan_{{ $key }}" name="jurusan" value="{{ $key }}" {{ old('jurusan', $rombonganBelajar->jurusan) === (string) $key ? 'checked' : '' }} required>
