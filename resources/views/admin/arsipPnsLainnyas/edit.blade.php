...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.arsipPnsLainnya.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.arsip-pns-lainnyas.update", [$arsipPnsLainnya->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('nama_ptk') ? 'is-invalid' : '' }}" name="nama_ptk_id" id="nama_ptk_id" required>
                    @foreach($nama_ptks as $id => $entry)
                        <option value="{{ $id }}" {{ (old('nama_ptk_id') ? old('nama_ptk_id') : $arsipPnsLainnya->nama_ptk->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('no_karpeg') ? 'is-invalid' : '' }}" type="text" name="no_karpeg" id="no_karpeg" value="{{ old('no_karpeg', $arsipPnsLainnya->no_karpeg) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('no_karis_karsu') ? 'is-invalid' : '' }}" type="text" name="no_karis_karsu" id="no_karis_karsu" value="{{ old('no_karis_karsu', $arsipPnsLainnya->no_karis_karsu) }}">
