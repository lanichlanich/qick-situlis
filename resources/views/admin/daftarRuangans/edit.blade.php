...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.daftarRuangan.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.daftar-ruangans.update", [$daftarRuangan->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="nama_ruangan">{{ trans('cruds.daftarRuangan.fields.nama_ruangan') }}</label>
                <input class="form-control {{ $errors->has('nama_ruangan') ? 'is-invalid' : '' }}" type="text" name="nama_ruangan" id="nama_ruangan" value="{{ old('nama_ruangan', $daftarRuangan->nama_ruangan) }}" required>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\DaftarRuangan::KONDISI_RUANGAN_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('kondisi_ruangan') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="kondisi_ruangan_{{ $key }}" name="kondisi_ruangan" value="{{ $key }}" {{ old('kondisi_ruangan', $daftarRuangan->kondisi_ruangan) === (string) $key ? 'checked' : '' }} required>
