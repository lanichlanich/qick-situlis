...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.peminjamanBuku.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.peminjaman-bukus.update", [$peminjamanBuku->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('peminjam_buku') ? 'is-invalid' : '' }}" name="peminjam_buku_id" id="peminjam_buku_id" required>
                    @foreach($peminjam_bukus as $id => $entry)
                        <option value="{{ $id }}" {{ (old('peminjam_buku_id') ? old('peminjam_buku_id') : $peminjamanBuku->peminjam_buku->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('nama_buku') ? 'is-invalid' : '' }}" name="nama_buku_id" id="nama_buku_id" required>
                    @foreach($nama_bukus as $id => $entry)
                        <option value="{{ $id }}" {{ (old('nama_buku_id') ? old('nama_buku_id') : $peminjamanBuku->nama_buku->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('tempat_penyimpanan_buku') ? 'is-invalid' : '' }}" name="tempat_penyimpanan_buku_id" id="tempat_penyimpanan_buku_id" required>
                    @foreach($tempat_penyimpanan_bukus as $id => $entry)
                        <option value="{{ $id }}" {{ (old('tempat_penyimpanan_buku_id') ? old('tempat_penyimpanan_buku_id') : $peminjamanBuku->tempat_penyimpanan_buku->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('jumlah_pinjam') ? 'is-invalid' : '' }}" type="number" name="jumlah_pinjam" id="jumlah_pinjam" value="{{ old('jumlah_pinjam', $peminjamanBuku->jumlah_pinjam) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('tanggal_pinjam') ? 'is-invalid' : '' }}" type="text" name="tanggal_pinjam" id="tanggal_pinjam" value="{{ old('tanggal_pinjam', $peminjamanBuku->tanggal_pinjam) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('tanggal_pengembalian') ? 'is-invalid' : '' }}" type="text" name="tanggal_pengembalian" id="tanggal_pengembalian" value="{{ old('tanggal_pengembalian', $peminjamanBuku->tanggal_pengembalian) }}" required>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\PeminjamanBuku::STATUS_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('status') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="status_{{ $key }}" name="status" value="{{ $key }}" {{ old('status', $peminjamanBuku->status) === (string) $key ? 'checked' : '' }} required>
