...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.daftarSiswa.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.daftar-siswas.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="no_induk">{{ trans('cruds.daftarSiswa.fields.no_induk') }}</label>
                <input class="form-control {{ $errors->has('no_induk') ? 'is-invalid' : '' }}" type="text" name="no_induk" id="no_induk" value="{{ old('no_induk', '') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nama_siswa') ? 'is-invalid' : '' }}" type="text" name="nama_siswa" id="nama_siswa" value="{{ old('nama_siswa', '') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('tgl_masuk') ? 'is-invalid' : '' }}" type="text" name="tgl_masuk" id="tgl_masuk" value="{{ old('tgl_masuk') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('asal_sekolah') ? 'is-invalid' : '' }}" name="asal_sekolah_id" id="asal_sekolah_id" required>
                    @foreach($asal_sekolahs as $id => $entry)
                        <option value="{{ $id }}" {{ old('asal_sekolah_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('tgl_keluar') ? 'is-invalid' : '' }}" type="text" name="tgl_keluar" id="tgl_keluar" value="{{ old('tgl_keluar') }}">
