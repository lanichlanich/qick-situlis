...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.daftarInventarisBarang.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.daftar-inventaris-barangs.update", [$daftarInventarisBarang->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('nama_barang') ? 'is-invalid' : '' }}" name="nama_barang_id" id="nama_barang_id" required>
                    @foreach($nama_barangs as $id => $entry)
                        <option value="{{ $id }}" {{ (old('nama_barang_id') ? old('nama_barang_id') : $daftarInventarisBarang->nama_barang->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('jumlah') ? 'is-invalid' : '' }}" type="number" name="jumlah" id="jumlah" value="{{ old('jumlah', $daftarInventarisBarang->jumlah) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('daftar_ruangan') ? 'is-invalid' : '' }}" name="daftar_ruangan_id" id="daftar_ruangan_id" required>
                    @foreach($daftar_ruangans as $id => $entry)
                        <option value="{{ $id }}" {{ (old('daftar_ruangan_id') ? old('daftar_ruangan_id') : $daftarInventarisBarang->daftar_ruangan->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\DaftarInventarisBarang::STATUS_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('status') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="status_{{ $key }}" name="status" value="{{ $key }}" {{ old('status', $daftarInventarisBarang->status) === (string) $key ? 'checked' : '' }} required>
