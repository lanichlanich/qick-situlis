...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.skKepangkatanPn.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.sk-kepangkatan-pns.update", [$skKepangkatanPn->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="no_surat">{{ trans('cruds.skKepangkatanPn.fields.no_surat') }}</label>
                <input class="form-control {{ $errors->has('no_surat') ? 'is-invalid' : '' }}" type="text" name="no_surat" id="no_surat" value="{{ old('no_surat', $skKepangkatanPn->no_surat) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('tgl_surat') ? 'is-invalid' : '' }}" type="text" name="tgl_surat" id="tgl_surat" value="{{ old('tgl_surat', $skKepangkatanPn->tgl_surat) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('nama_ptk') ? 'is-invalid' : '' }}" name="nama_ptk_id" id="nama_ptk_id" required>
                    @foreach($nama_ptks as $id => $entry)
                        <option value="{{ $id }}" {{ (old('nama_ptk_id') ? old('nama_ptk_id') : $skKepangkatanPn->nama_ptk->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('tmt_cpns') ? 'is-invalid' : '' }}" type="text" name="tmt_cpns" id="tmt_cpns" value="{{ old('tmt_cpns', $skKepangkatanPn->tmt_cpns) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('masa_kerja_golongan') ? 'is-invalid' : '' }}" type="number" name="masa_kerja_golongan" id="masa_kerja_golongan" value="{{ old('masa_kerja_golongan', $skKepangkatanPn->masa_kerja_golongan) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('masa_kerja_bulan') ? 'is-invalid' : '' }}" type="number" name="masa_kerja_bulan" id="masa_kerja_bulan" value="{{ old('masa_kerja_bulan', $skKepangkatanPn->masa_kerja_bulan) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('pangkat_golongan') ? 'is-invalid' : '' }}" name="pangkat_golongan" id="pangkat_golongan" required>
                    ...
                    @foreach(App\Models\SkKepangkatanPn::PANGKAT_GOLONGAN_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('pangkat_golongan', $skKepangkatanPn->pangkat_golongan) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
