...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.skPengangkatanHonorer.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.sk-pengangkatan-honorers.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('tmt_sk') ? 'is-invalid' : '' }}" type="text" name="tmt_sk" id="tmt_sk" value="{{ old('tmt_sk') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('masa_kerja') ? 'is-invalid' : '' }}" type="number" name="masa_kerja" id="masa_kerja" value="{{ old('masa_kerja', '') }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('jenis_ptk') ? 'is-invalid' : '' }}" name="jenis_ptk" id="jenis_ptk" required>
                    ...
                    @foreach(App\Models\SkPengangkatanHonorer::JENIS_PTK_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('jenis_ptk', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
